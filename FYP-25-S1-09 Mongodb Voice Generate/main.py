...
def send_assignment_notification():
    ...
    logger.debug(f"Assignments due between {today} and {two_days_after}")
    
    # Find assignments due in 1 or 2 days
    ...
    logger.debug(f"Found {assignments_cursor.count()} assignments in the date range.")

    for assignment in assignments_cursor:
        due_date = assignment.get('due_date')
        logger.debug(f"Checking assignment: {assignment.get('title')} with due date: {due_date}")

        if not due_date:
            logger.warning(f"Assignment '{assignment.get('title')}' has no due date, skipping.")
            continue
        ...
        if days_until_due not in [1, 2]:
            logger.debug(f"Skipping assignment '{assignment.get('title')}' as it is not due in 1 or 2 days.")
            continue

        message = f"Reminder: Assignment '{assignment.get('title')}' is due in {days_until_due} day(s)!"
        ...
        if not classroom or 'students' not in classroom:
            logger.warning(f"Classroom {classroom_id} does not exist or has no students, skipping.")
            continue
        ...
        for user_id in classroom['students']:
            ...
            logger.debug(f"Inserting notification for user {user_id}: {notification}")
