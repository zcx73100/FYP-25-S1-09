...
from . import mongo  # ✅ Absolute import
import os
...
API_URL = "http://127.0.0.1:7860"  # Local hosting for SadTalker 


# API_URL = "https://vinthony-sadtalker.hf.space/--replicas/li6va/"  online API
...
def process_video(video_id, text, avatar_path, audio_path):
    try:
        ...
        if not os.path.exists(avatar_path) or not os.path.exists(audio_path):
            print("❌ Error: Avatar or Audio file does not exist!")
            return None

        # SadTalker API request
        sad_talker_url = f"{API_URL}/gradio_api/run/generate_talking_video"

        print(f"🎬 Sending request to {sad_talker_url} with Avatar: {avatar_path} and Audio: {audio_path}")

        # Send request to SadTalker
        with open(avatar_path, "rb") as image_file, open(audio_path, "rb") as audio_file:
            files = {
                "image_file": ("avatar.jpg", image_file, "image/jpeg"),
                "audio_file": ("audio.wav", audio_file, "audio/wav"),
            }
            response = requests.post(sad_talker_url, files=files)

        if response.status_code != 200:
            print(f"❌ SadTalker API Error {response.status_code}: {response.text}")
            return None

        # Extract response
        try:
            result = response.json()
            video_path = result.get("video_path")  # Adjust if API response is different
            if not video_path:
                print("❌ No video path received from SadTalker!")
                return None
