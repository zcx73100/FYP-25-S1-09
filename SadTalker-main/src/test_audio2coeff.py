...
class Audio2Coeff():
    ...
    def __init__(self, sadtalker_path, device):
        #load config
        pose_path = sadtalker_path['audio2pose_yaml_path']
        print(f"[DEBUG] Attempting to load pose YAML from: {pose_path}")
        print(f"[DEBUG] Absolute path: {os.path.abspath(pose_path)}")

        fcfg_pose = open(pose_path)
