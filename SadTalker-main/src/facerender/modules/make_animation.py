...
PROGRESS_FILE = os.path.abspath("progress.txt") 
...
def make_animation(source_image, source_semantics, target_semantics,
    ...
    with torch.no_grad():
        ...
        # Step 1: Extract canonical keypoints
        ...
        # Step 2: For each frame in target semantics
        for frame_idx in tqdm(range(total_frames), 'Face Renderer:'):
            # ✅ Write progress to file for frontend polling
            with open(PROGRESS_FILE, "w") as f:
                f.write(f"{frame_idx + 1}/{total_frames}")

            target_semantics_frame = target_semantics[:, frame_idx]
            he_driving = mapping(target_semantics_frame)

            # Override with optional custom yaw/pitch/roll if provided
            ...
            # Transform target keypoints
            kp_driving = keypoint_transformation(kp_canonical, he_driving)
            kp_norm = kp_driving  # not using normalization here

            # Run generator
            out = generator(source_image, kp_source=kp_source, kp_driving=kp_norm)
            predictions.append(out['prediction'])

        # ✅ Final write: mark as done
        with open(PROGRESS_FILE, "w") as f:
            f.write("done")
