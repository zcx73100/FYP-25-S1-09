...
class SadTalker():
    ...
    def __init__(self, checkpoint_path='checkpoints', config_path='src/config', lazy_load=False):
        ...
        print(f"[INFO] SadTalker using device: {self.device}")
        
        os.environ['TORCH_HOME'] = checkpoint_path
        self.checkpoint_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', checkpoint_path))
    ...
    def test(self, source_image, driven_audio, preprocess='crop', 
        ...
        try:
            ...
            print("[DEBUG] Copying avatar to input directory...")
            pic_path = os.path.join(input_dir, os.path.basename(source_image))
            shutil.copy(source_image, pic_path)
            print(f"[DEBUG] Copied image to: {pic_path}")
            ...
            if driven_audio is not None and os.path.isfile(driven_audio):
                ...
                if '.mp3' in audio_path:
                    ...
                    print("[DEBUG] Copying audio to input directory...")
                    shutil.copy(driven_audio, audio_path)
                    print(f"[DEBUG] Copied audio to: {audio_path}")
